 ...
 */
async function handleWorkoutIngest(req, res) {
    ...
    const clientValidation = await validateClientId(clientId);
    ...
    for (const workout of workouts) {
        ...
        const isDup = await isDuplicateWorkout(workout);
        if (isDup) {
            duplicates++;
            ...
            await createWarning(clientId, 'workout', null, 'duplicate', 
        ...
        // Insert workout
        try {
            const result = await insertWorkout(workout);
            ...
            if (validation.warnings.length > 0 && result.lastInsertRowid) {
                for (const warning of validation.warnings) {
                    await createWarning(clientId, 'workout', result.lastInsertRowid, 'validation', warning);
                }
            }
            inserted++;
        } catch (error) {
 ...
 */
async function handleProfileIngest(req, res) {
    const startTime = new Date();
    const timestamp = startTime.toISOString();
    ...
    for (const metric of metrics) {
        ...
        if (validation.warnings.length > 0) {
            warnings += validation.warnings.length;
            for (const warning of validation.warnings) {
                await createWarning(clientId, 'profile_metric', null, 'validation', warning);
            }
        }
        
        // Insert metric
        try {
            const result = await insertProfileMetric(metric);
            // Store record ID for warning tracking
            if (validation.warnings.length > 0 && result.lastInsertRowid) {
                for (const warning of validation.warnings) {
                    await createWarning(clientId, 'profile_metric', result.lastInsertRowid, 'validation', warning);
                }
            }
            inserted++;
        } catch (error) {
 ...
 */
router.post('/workouts', asyncHandler(handleWorkoutIngest));

/**
 * POST /ingest/profile or POST /api/v1/profile-metrics
 * Accepts an array of profile metric objects and inserts them
 */
router.post('/profile', asyncHandler(handleProfileIngest));
router.post('/profile-metrics', asyncHandler(handleProfileIngest));

/**
 * DELETE /workouts or DELETE /api/v1/workouts
 * Delete workouts by UUIDs
 */
router.delete('/workouts', asyncHandler(async (req, res) => {
    ...
    const deletedCount = await deleteWorkoutsByUuids(uuids);
 ...
 */
router.delete('/profile-metrics', asyncHandler(async (req, res) => {
    const { uuids } = req.body;
    ...
    const deletedCount = await deleteProfileMetricsByUuids(uuids);
