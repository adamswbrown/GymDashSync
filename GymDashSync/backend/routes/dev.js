...
router.post('/seed', (req, res) => {
    try {
        ...
        createClient(client1Id, code1, 'Test Client 1');
        createClient(client2Id, code2, 'Test Client 2');
...
router.get('/health', (req, res) => {
    try {
        ...
        const warningsStmt = db.prepare('SELECT COUNT(*) as count FROM warnings');
        const warningsCount = warningsStmt.get().count;
        ...
        res.status(200).json({
            ...
            assumptions: {
                ...
                clients_with_data: clients.filter(c => (c.workouts_count || 0) > 0).length
            },
